...
export class Indiware {
  ...
  private static parseKopf(plan: IndiwareResult): ScheduleResultHead {
    ...
    return {
      ...
      skipDates: FreieTage?.[0].ft.map((ft) =>
        this.parseShortDateString(ft).toUTCString()
      ),
    };
  }

  private static parseKlassen(
    plan: IndiwareResult,
    options: ScheduleOptions<IndiwareOptions>
  ): ScheduleResultBody {
    const {
  ...
  private static parseSupervisions(
    plan: IndiwareResult,
    options: ScheduleOptions<IndiwareOptions>
  ) {
